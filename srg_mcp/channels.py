...
def list_channels(hub_profile_id: str, include_archived: bool = False) -> list[dict]:
    ...
    return [
        ...
        for c in get_client().channels.list(
            hub_profile_id, include_archived=include_archived
        )
    ]
...
def get_channel_by_name(hub_profile_username: str, channel_name: str) -> dict:
    ...
    return (
        get_client()
        .channels.get_by_name(hub_profile_username, channel_name)
...
def get_category_by_slugs(
    ...
    return (
        get_client()
        .channels.get_category_v2(hub_profile_username, channel_name, category_name)
        .model_dump(mode="json")
    )
...
def update_section(
    ...
    return get_client().channels.update_section(
        channel_id, category_id, section_id, name=name
    )
